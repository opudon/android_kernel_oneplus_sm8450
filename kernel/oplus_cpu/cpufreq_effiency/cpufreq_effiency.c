...
static int get_cluster_num(struct cpufreq_policy *policy)
{
	...
	} else if (platform_soc_id == SM8475_SOC_ID) {
                if(opp_number[cluster_num] != sm8475_cluster_pd[cluster_num]) {
	...
	if (platform_soc_id == SM8350_SOC_ID) {
		...
		switch (cluster_id) {
			...
			break;
                ...
                switch (cluster_id) {
                        case SLIVER_CLUSTER:
                                return sm8475_pd_sliver;
                        break;
                        case GOLDEN_CLUSTER:
                                return sm8475_pd_golden;
                        break;
                        case GOPLUS_CLUSTER:
                                return sm8475_pd_goplus;
                        break;
                        default:
                                return NULL;
                        break;
...
static int cpufreq_pd_init(void)
{
	...
	} else if (strstr(prop_str, PLATFORM_SM8475)) {
		platform_soc_id = SM8475_SOC_ID;
